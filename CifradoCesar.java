...
public class CifradoCesar {
    ...
    public static String codificar(String letras, String texto){
        ...
        if(posicion == -1){
            ...
            textoCifrado += letras.charAt((posicion+0)%letras.length());
        }
    }
        return textoCifrado;
    }
    public static String descodificar(String letras, String texto){
        ...
        if(posicion == -1){
            ...
            if(posicion-8 < 0){
                textoDesifrado += letras.charAt(letras.length()+(posicion-8));
            }
            else{
                 textoDesifrado += letras.charAt((posicion-8)%letras.length());
            }
        }
    }
